...
func solvePuzzle(input: [String]) -> Int {
    ...
    for update in updates {
        ...
        if !orderedRight {
            var newUpdate = update.components(separatedBy: ",")
            var correctOrder: Bool = false
            while !correctOrder {
                correctOrder = true
                outerLoop: for i in 0..<pages.count {
                    for rule in rules {
                        if newUpdate[i] == String(rule.components(separatedBy: "|")[0])
                            && newUpdate.contains(String(rule.components(separatedBy: "|")[1]))
                        {
                            if !newUpdate[i...newUpdate.count - 1].joined(separator: ",").contains(
                                String(rule.components(separatedBy: "|")[1]))
                            {
                                correctOrder = false
                                let page = newUpdate.remove(at: i)
                                newUpdate.insert(page, at: i - 1)
                                break outerLoop
                            }
                        }
                    }
            ...
            numberStore.append(Int(newUpdate[newUpdate.count / 2])!)
        }
    }
